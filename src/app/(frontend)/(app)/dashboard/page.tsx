    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Track topic intake, review, readiness, posting, and declines.</p>
        </div>
        <Button asChild>
            ...
            <div className="space-y-4">
              {recentActions.docs.length === 0 ? (
                ...
                recentActions.docs.map((action) => (
                  <div className="min-w-0 border-l-2 border-slate-200 pl-4" key={action.id}>
                    <div className="flex items-center justify-between gap-3">
                      <p className="truncate text-sm font-medium text-slate-950">{userName(action.performedBy)}</p>
                    ...
                    {action.comment ? (
                      <p className="mt-1 truncate text-xs text-slate-500" title={action.comment}>
                        {action.comment}
                      </p>
                    ) : null}
                  </div>
